...
$('crew-list').onclick=e=>{const button=e.target.closest('[data-dismiss-pilot]');if(!button)return;const pilot=view?.players.find(p=>p.id===button.dataset.dismissPilot);if(!pilot||pilot.connected)return;openModal('dismiss','COMMANDER / RELEASE OFFLINE SEAT','<h2>Make room for your crew.</h2><p>Release '+esc(pilot.name)+'’s offline seat? Their saved identity will no longer rejoin this seat. They can explicitly join again as a new pilot if a seat is available.</p><button id="dismiss-confirm" class="primary">RELEASE OFFLINE SEAT</button><button id="dismiss-cancel" class="secondary">KEEP RESERVED</button>');$('dismiss-confirm').onclick=()=>{action({type:'dismiss',playerId:pilot.id});closeModal();};$('dismiss-cancel').onclick=closeModal;};
...
function radar(){const p=self();if(!p)return;const ctx=$('radar').getContext('2d'),size=180,c=90,range=p.zone==='space'?180:70;ctx.clearRect(0,0,size,size);ctx.strokeStyle='#8ecbd62a';ctx.lineWidth=1;for(const r of [30,60,88]){ctx.beginPath();ctx.arc(c,c,r,0,Math.PI*2);ctx.stroke();}ctx.beginPath();ctx.moveTo(c,0);ctx.lineTo(c,180);ctx.moveTo(0,c);ctx.lineTo(180,c);ctx.stroke();
 ...
 view.interactables.filter(q=>q.zone===p.zone&&!q.id.startsWith('own-ship')).forEach(q=>dot(q,'#ffb86d',3));view.npcs.filter(q=>q.zone===p.zone&&q.alive).forEach(q=>dot(q,'#ff6c64',3));view.players.filter(q=>q.id!==you&&q.zone===p.zone&&q.alive).forEach(q=>dot(q,view.mode==='expedition'?'#82e5eb':'#ff6c64',4));ctx.fillStyle='#fff';ctx.beginPath();ctx.moveTo(90,83);ctx.lineTo(85,96);ctx.lineTo(95,96);ctx.fill();}
