import {SCORES,scheduleScore,scoreKey} from './audio-score.js?v=11';
...
export class AudioEngine {
 constructor(){this.context=null;this.scene='menu';this.musicVolume=.32;this.sfxVolume=.65;this.muted=false;this.destroyed=false;this.visible=true;this.reducedMotion=false;this.intensity=0;this.musicVoices=new Set();this.effectVoices=new Set();this.cooldowns=new Map();this.timer=null;this.epoch=0;this.stage=null;this.score=scoreKey(this.scene);this.transport=null;this.pendingScore=null;this.pendingStep=0;this.motion=null;this.motionTimer=null;this.nextStep=0;this.stepSide=0;}
 ...
 setScene(name){
  ...
  if(key===this.score){if(this.pendingScore){this.pendingScore=null;const g=this.stage.dry.gain,t=this.context.currentTime;if(g.cancelAndHoldAtTime)g.cancelAndHoldAtTime(t);else{g.cancelScheduledValues(t);g.setValueAtTime(g.value,t);}g.linearRampToValueAtTime(1,t+.35);}return;}
  if(key===this.pendingScore)return;this.pendingScore=key;this.pendingStep=Math.ceil((this.transport.step+1)/128)*128;this._scheduleHandoff();
 }
 _scheduleHandoff(){
  if(!this.pendingScore||!this.transport||!this.stage)return;const c=this.context,tr=this.transport,b=60/SCORES[this.score].bpm,t=tr.nextTime+(this.pendingStep-tr.step)*b/4,g=this.stage.dry.gain,now=c.currentTime;
 ...
 _stopScore(){this.epoch++;if(this.timer)clearTimeout(this.timer);this.timer=null;this.transport=null;this.pendingScore=null;this.score=scoreKey(this.scene);for(const voice of [...this.musicVoices])voice.stop();if(this.stage){for(const node of this.stage.nodes)try{node.disconnect();}catch{}this.stage=null;}}
 _newStage(){const c=this.context,dry=c.createGain(),verb=c.createConvolver(),wet=c.createGain(),echo=c.createDelay(1),echoGain=c.createGain(),feedback=c.createGain(),filter=c.createBiquadFilter();const mythic=this.score==='mythic';verb.buffer=mythic?this.musicImpulse:this.impulse;wet.gain.value=mythic?.3:.27;echo.delayTime.value=60/SCORES[this.score].bpm*.75;echoGain.gain.value=mythic?.075:.11;feedback.gain.value=mythic?.14:.18;filter.type='lowpass';filter.frequency.value=mythic?1800:2200;dry.connect(this.musicBus);verb.connect(wet);wet.connect(dry);echo.connect(filter);filter.connect(echoGain);echoGain.connect(dry);filter.connect(feedback);feedback.connect(echo);this.stage={dry,verb,echo,wet,echoGain,feedback,filter,mythic,nodes:[dry,verb,wet,echo,echoGain,feedback,filter]};}
 _stageColor(score,at){const stage=this.stage,mythic=score==='mythic';if(stage.mythic===mythic)return;stage.mythic=mythic;stage.verb.buffer=mythic?this.musicImpulse:this.impulse;stage.wet.gain.setValueAtTime(mythic?.3:.27,at);stage.echoGain.gain.setValueAtTime(mythic?.075:.11,at);stage.feedback.gain.setValueAtTime(mythic?.14:.18,at);stage.filter.frequency.setValueAtTime(mythic?1800:2200,at);}
 _startScore(){
  ...
  const tick=()=>{if(this.destroyed||epoch!==this.epoch)return;const tr=this.transport;if(c.state==='running'){
   ...
   while(tr.nextTime<c.currentTime+.17){
    if(this.pendingScore&&tr.step>=this.pendingStep){for(const v of [...this.musicVoices])v.endAt(tr.nextTime);this.score=this.pendingScore;this.pendingScore=null;tr.scoreStart=tr.step;this.stage.echo.delayTime.setValueAtTime(60/SCORES[this.score].bpm*.75,tr.nextTime);this._stageColor(this.score,tr.nextTime);}
 ...
 get diagnostics(){return{scene:this.scene,score:this.score,pendingScore:this.pendingScore,scoreStep:this.transport?this.transport.step-this.transport.scoreStart:0,visible:this.visible,activeScores:this.stage?1:0,musicVoices:this.musicVoices.size,effectVoices:this.effectVoices.size,motionActive:!!this.motion,state:this.context?.state||'locked',destroyed:this.destroyed};}
